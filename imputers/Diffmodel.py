...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels, 
        ...
        self.encoderlayer=nn.TransformerEncoderLayer(d_model=512,nhead=4,dim_feedforward=512,dropout=0)
        self.encoder = nn.TransformerEncoder(self.encoderlayer,num_layers=1)
        
        self.encoderlayer2=nn.TransformerEncoderLayer(d_model=512,nhead=4,dim_feedforward=512,dropout=0)
        self.encoder2 = nn.TransformerEncoder(self.encoderlayer,num_layers=1)
    ...
    def forward(self, input_data):
        ...
        h = self.encoder2(h.permute(2,0,1)).permute(1,2,0) 
        ...
        return (x + res) * math.sqrt(0.5), skip,cond  # normalize for training stability
...
class Residual_group(nn.Module):
    ...
    def forward(self, input_data):
        ...
        skip = 0
        for n in range(self.num_res_layers):
            h, skip_n ,cond= self.residual_blocks[n]((h, conditional, diffusion_step_embed))  
            skip = skip_n  + skip

        return skip * math.sqrt(1.0 / self.num_res_layers)  ,cond
...
class Diffmodel(nn.Module):
    ...
    def forward(self, input_data):
        ...
        x = noise
        x = self.init_conv(x)
        x,cond = self.residual_layer((x, conditional, diffusion_steps))
        y = self.final_conv(x)
